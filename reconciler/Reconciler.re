...
module Render = {
  ...
  module OpaqueInstanceHash = {
    let addOpaqueInstances = (idTable, opaqueInstances) => {
      let add = opaqueInstance => {
        let Instance({component: {key, _}, _}) = opaqueInstance;
  ...
  let rec renderElement = (~useKeyTable=?, element): opaqueInstance =>
    ...
    | Element(component) =>
      let opaqueInstanceFromKey =
        ...
        | None => None
        | Some(keyTable) =>
          OpaqueInstanceHash.lookupKey(keyTable, component.key)
        };
  ...
  and update =
    ...
    let bailOut = {
      let Instance({element, component, _}) = opaqueInstance;
      let log = () => {
        logString(
    ...
    let logUpdate =
      ...
      let Instance({id, _}) = opaqueInstance;
      let Instance({id: newId, _}) = newOpaqueInstance;
    ...
    if (bailOut) {
      ...
      let Instance(
            {component, instanceSubTree, subElements, _} as updatedInstance,
          ) = updatedOpaqueInstance;
      switch (nextElement) {
      | Element(nextComponent) =>
        component.handedOffInstance := Some(updatedInstance) |> Obj.magic;
        ...
        | Some(updatedInstance) =>
          ...
          let stateChanged =
            ...
            !stateNotUpdated || newState !== Obj.magic(updatedInstance).iState;
          ...
          if (nextComponent.shouldUpdate === defaultShouldUpdate
            ...
            let subTreeChanged = {
              let Instance({instanceSubTree, _}) = opaqueInstance;
  ...
  and updateRenderedElement =
     ...
     */
    let processElement =
        (~keyTable, ~posTable, position, element): opaqueInstance =>
      switch (element) {
  ...
  let flushPendingUpdates = (~updateLog, opaqueInstance) => {
    let Instance({element, _}) = opaqueInstance;
    update(
      ~updateLog,
...
module OutputTree = {
  ...
  let rec fromOpaqueInstance =
          (Instance({component, element, iState, instanceSubTree, id, _}))
          : tree =>
    switch (element) {
    | String(s) => S(s)
    | Element(_) =>
  ...
  let applyUpdateLog = (updateLog, forest) => {
      ...
      | UpdateInstance({
        ...
        | N(n) =>
          if (n.id === oldId) {
            ...
            let Instance({component, iState, instanceSubTree, _}) = newOpaqueInstance;
            if (componentChanged) {
            ...
            if (subTreeChanged) {
              let Instance({instanceSubTree: oldInstanceSubTree, _}) = oldOpaqueInstance;
              let sub =
    ...
    let applyEntryForestToplevel = (f, entry) =>
      switch (entry) {
      | NewRenderedElement(renderedElement) =>
        Some(fromRenderedElement(renderedElement))
  ...
  let print = (treeList: forest) => {
    ...
    let l: list(string) =
      List.mapi(
        (i, tree) =>
          "Tree #" ++ string_of_int(i) ++ " " ++ arrayStr([printTree(tree)]),
        treeList,
      );
    String.concat("\n", l);
  };
